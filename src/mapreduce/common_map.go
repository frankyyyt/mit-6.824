...
func doMap(
	...
	//
	fmt.Printf("domap %s -maptask-%d -reduceTasks-%d\n",jobName,mapTaskNumber,nReduce)
	data,_ := ioutil.ReadFile(inFile)

	kvs := mapF(inFile,string(data))
	mapEnc :=make(map[int]*json.Encoder)
	for _,kv := range kvs{
		hv :=ihash(kv.Key)%nReduce
		enc,ok := mapEnc[hv]
		if ok{
			enc.Encode(kv)
		}else{
			reduceFile :=reduceName(jobName,mapTaskNumber,hv)
			f,_ :=os.Create(reduceFile)
			defer f.Close()
			enc =json.NewEncoder(f)
			enc.Encode(kv)
			f.Sync()
			mapEnc[hv]=enc
			//fmt.Printf("map wirte enc:%s\n",kv.Key,kv.Value)
		}
	}
